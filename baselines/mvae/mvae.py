...
from .model import GeneratorX, GeneratorY, InferenceX, InferenceY
import datetime
...
class MVAE():
    ...
    def init_model(self):
        ...
        q = ProductOfNormal([self.q_x, self.q_y], name="q").to(self._device)
        ...
        ELBO_x = -E(self.q_x, LogProb(self.p_x)) + KullbackLeibler(self.q_x, prior)
        ELBO_y = -E(self.q_y, LogProb(self.p_y)) + KullbackLeibler(self.q_y, prior)
        loss_fn = ELBO.mean() + ELBO_x.mean() + ELBO_y.mean()
        self._model = Model(loss=loss_fn, distributions=[self.p_x, self.p_y, self.q_x, self.q_y], optimizer=optim.Adam, optimizer_params={"lr":1e-3})
        self.q = q
